...
export default {
  ...
  methods: {
    ...
    RemoveDeal(id){
      this.deals.splice(this.deals.findIndex(item => item.id === id), 1);
    }
  },
  computed: {
    showRouterView() {
      return !['/loginpage', '/registerpage', '/goals'].includes(this.$route.path);
    }
  }
};
</script>
  ...
  <div v-if="showRouterView">
    ...
    <div style="display: flex; flex-direction: column;">
      <div style="display: flex; flex-direction: row; ">
          <input v-model="inputDesc" id="inputDesc" style="color: antiquewhite;" type="text" placeholder="Введите текст задачи" />
          <v-combobox
          label="Степень важности задачки"
          :items="['Важно до безумия', 'Важно', 'Терпица слюбица', 'На потом', 'Забей ваще', 'НЕ ДЕЛАТЬ']" v-model="importance" style="min-width: 200px;">
          </v-combobox>
          
        <DatePicker v-model="date" dateFormat="dd/mm/yy" />
        <v-date-input label="Дата постановки" v-model="date" style="min-width: 200px;"></v-date-input>
      </div>
      <div>
